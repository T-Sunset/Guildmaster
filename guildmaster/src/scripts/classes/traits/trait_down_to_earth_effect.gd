# NAME.GD
extends Effect
class_name TraitDownToEarthEffect

# Constructor
func _init():
	super("Down to Earth", "This hero is +20% more effective per rank in quests tagged 'Underground'.", true)
...
func _get_aptitude_modifier(quest:QuestData) -> float: # Does this effect modify this hero's output aptitude
	...
	if quest.get_tags().has("underground"):
		return val
	else:
		return 1.0
