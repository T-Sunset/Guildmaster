# NAME.GD
extends Effect
class_name TraitHumanoidSlayerEffect

# Constructor
func _init():
	super("Humanoid Slayer", "This hero is +20% more effective per rank in quests tagged 'Humanoid'.", true)
...
func _get_aptitude_modifier(quest:QuestData) -> float: # Does this effect modify this hero's output aptitude
	...
	if quest.get_tags().has("humanoid"):
		return val
	else:
		return 1.0
